...
import { BookOutlined, FileProtectOutlined, UserOutlined, MenuFoldOutlined, MenuUnfoldOutlined, LogoutOutlined, HomeOutlined } from '@ant-design/icons';
...
const StudentLayout = () => {
  ...
  const [userInfo, setUserInfo] = useState({ name: 'Sinh viên', avatar: null });
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      ...
      >
        <div style={{ height: 64, margin: 16, display: 'flex', justifyContent: 'center', alignItems: 'center', fontWeight: 'bold', color: 'white', fontSize: collapsed ? '14px' : '16px' }}>{collapsed ? 'SV' : 'Sinh viên'}</div>
        <Menu 
          theme="dark" 
          mode="inline" 
          ...
          items={[
            ...
            { key: '/student/classes', icon: <BookOutlined />, label: 'Lớp học của tôi' },
            { key: '/student/grades', icon: <FileProtectOutlined />, label: 'Bảng điểm' },
          ]}
        />
      </Sider>
      <Layout>
          ...
          <Space size="large">
            ...
            >
              <Space style={{ cursor: 'pointer', color: 'white' }}>
                <Avatar style={{ backgroundColor: '#87d068' }} icon={<UserOutlined />} src={userInfo.avatar} />
